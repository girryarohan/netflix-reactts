...
export default function BrowseContainer({ slides }: IBrowseContainerProps) {
  ...
  const shouldSlidesUpdate = (results: ISlideRows[]) =>
    slideRows.length > 2 && results.length > 0
  ...
  return "displayName" in profile && profile.displayName ? (
    <>
      ...
      <Header src="joker1" dontShowOnSmallViewPort>
        <Header.Frame>
          <Header.Group>
            <Header.Logo alt="Netflix" to={RouteConstants.HOME} src={logo} />
            <Header.TextLink
        ...
        {slideRows.map((slideItem) => (
          <Card key={`${category}-${slideItem.title.toLowerCase()}`}>
            ...
            <Card.Feature category={category}>
              <Player>
                <Player.Button />
                <Player.Video src={`/videos/tmkoc.mp4`} />
              </Player>
            </Card.Feature>
          </Card>
        ))}
